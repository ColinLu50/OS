...
#define PROCESS_NAME_LEN 32 //进程名最大长�?
#define MIN_SLICE 10 //内�?片最大大�?
#define DEFAULT_MEM_SIZE 1024  //总内存大�?
#define DEFAULT_MEM_START 0  //内存开始分配时的起始地址

typedef pair<int, string> My_algo;

int mem_size = DEFAULT_MEM_SIZE;
bool flag = 0; //当内存以及�?分配了之后，不允许更改总内存大小的flag
static int pid = 0;
My_algo algo[123];

struct free_block{	//空闲数据�?
	int size;
	int start_addr;
	struct free_block *next;
};

struct allocated_block{ //已分配的数据�?
	int pid;
	int size;
	int start_addr;
...
free_block *free_block_head; //空闲数据块�?指针
allocated_block *allocated_block_head = NULL; //分配块�?指针
...
void rearrange(); // 对块进�?重新分配
int create_new_process(); //创建新的进程
int free_mem(allocated_block *ab); //释放分配�?
void swap(int *p, int *q); //交换地址
int dispose(allocated_block *ab); //释放分配块结构体
void display_mem_usage(); //显示内存情况
void kill_process(); //杀死�?应进程并释放其空间与结构�?
void Usemy_algo(int id); //使用对应的分配算�?

//主函�?
int main(){
	int op;
	pid = 0;
	free_block_head = init_free_block(mem_size); //初�?化一�?��以使用的内存块，类似与操作系统可用的总存储空�?
	for(;;){
		sleep(1);
		display_menu();
		fflush(stdin);
...
allocated_block *find_process(int id){ //�?��遍历分配块链�?��寻找pid=id的进程所对应的块
}

free_block *init_free_block(int mem_size){ //初�?化空闲块，这里的mem_size表示允�?的最大虚拟内存大�?
	free_block *p;
...
void set_mem_size(){ //更改最大内存大�?
}

int allocate_mem(allocated_block *ab){ //为块分配内存，真正的操作系统会在这里进�?�?��等操�?
}

int create_new_process(){ //创建新进�?
}
