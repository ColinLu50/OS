...
int clock_alg() {
    ...
    unordered_map<int, CircularNode*> in_cache_page;
    int curr;
...
int FIFO() {
    unordered_map<int, int> used_pages;
    queue<int> q;
    int curr;
    for(int i = 0; i < page_num; i ++) {
        cin >> curr;
...
int LRU() {
    list<int> use;
    unordered_map<int, list<int>::iterator> in_cache_page_to_list_pos;
...
int second_chance() {
    ...
    list<int> LRU_list;
    unordered_map<int, list<int>::iterator> in_LRU_to_list_pos;
    unordered_map<int, int> in_FIFO;
    queue<int> q;
...
int min() {
    unordered_map<int, queue<int>> use;
    unordered_map<int, int> in_cache;
    int in_cache_num = 0;
...
int main() {
    ...
    default:
        printf("Your algorithme number is wrong!\n");
        return 1;
        break;
    }

    printf("Hit ratio = %2.2f%%\n", 100*hit_num/(double)page_num);
